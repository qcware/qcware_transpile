...
def ir_instruction_from_native(
    gate: qiskit.circuit.Gate, qubits: List[int], clbits: List[int]
) -> Instruction:
    return Instruction(
        ...
        metadata={"clbits": pvector(clbits)} if len(clbits) > 0 else {},
    )  # type: ignore
...
def native_circuits_are_equivalent(
    ...
    """
    instruction_pairs = zip(normalized_instructions(c1), normalized_instructions(c2))

import pyzx
from hypothesis.strategies import (lists, sampled_from, booleans, composite,
                                   floats, integers)
...
@composite
def gates(draw,
          possible_qubits: Set[int],
          gate_list=sorted(pyzx_dialect.dialect().gate_defs)):
    ...
    qubits = draw(
        lists(sampled_from(sorted(list(possible_qubits))),
              unique=True,
              min_size=num_qubits,
              max_size=num_qubits))
    ...
    try:
        ...
        raise ValueError(
            f"Invalid number of qubits {qubits} for gate {gate_def.name}")
...
def circuits(
    ...
    circuit_gates = draw(
        lists(gates(possible_qubits=range(num_qubits)),
              min_size=length,
              max_size=length))
